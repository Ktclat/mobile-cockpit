...
enum class ConversationSendRouteStatus { READY, MODEL_ROUTE_MISSING, MODEL_ROUTE_REVISION_MISMATCH }

fun interface ConversationSendRouteGuard {
    suspend fun evaluate(conversationId: ConversationId, agentId: AgentId): ConversationSendRouteStatus
}

enum class SendConversationMessageResult {
    Sent,
    Rejected,
    MODEL_ROUTE_MISSING,
...
class SendConversationMessage(
    ...
    private val routeGuard: ConversationSendRouteGuard,
) {
    suspend operator fun invoke(destination: ConversationMessageDestination, text: String): SendConversationMessageResult {
        ...
        when (routeGuard.evaluate(destination.conversationId, snapshot.conversation.conversation.agentId)) {
            ConversationSendRouteStatus.READY -> Unit
            ConversationSendRouteStatus.MODEL_ROUTE_MISSING ->
                return SendConversationMessageResult.MODEL_ROUTE_MISSING
            ConversationSendRouteStatus.MODEL_ROUTE_REVISION_MISMATCH ->
                return SendConversationMessageResult.MODEL_ROUTE_REVISION_MISMATCH
        }
        ...
        repository.save(snapshot.copy(conversation = snapshot.conversation.copy(conversation = accepted), messages = snapshot.messages + MessagePersistenceState(ids.nextId(), message, ordinal, MessageRole.USER, MessageSource.USER, MessageStatus.ACCEPTED), drafts = snapshot.drafts.filterNot { it.destination == destination }))
