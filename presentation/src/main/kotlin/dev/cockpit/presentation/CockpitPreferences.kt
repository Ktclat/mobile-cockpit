...
private val ChineseTranslations = mapOf(
    ...
    "Response interrupted" to "回复已中断",
    "Provider response failed" to "模型服务响应失败",
    "The reply was saved, but its generation status could not be finalized." to "回复已保存，但生成状态未能完成记录。",
    ...
    "Accept the updated API configuration" to "接受此 API 配置的新版本",
    ...
    "The configuration transaction failed. The previous configuration remains unchanged." to "配置保存事务失败，原配置保持不变。",
    "The configuration transaction failed, and the unused new local credential could not be cleaned up." to "配置保存事务失败，且未使用的新本地密钥暂时无法清理。",
    "Configuration saved, but the previous local credential could not be cleaned up. Retry deletion later; the new configuration will not fall back to the old credential." to "配置已保存，但旧的本地密钥暂未清理。请稍后重试删除；新配置不会回退到旧密钥。",
    "Configuration deleted, but its unused local credential could not be cleaned up." to "配置已删除，但未使用的本地密钥暂未清理。",
    "The previous response did not finish after the app was interrupted." to "上次回复在应用中断后未完成。",
    "The previous provider response failed before it could be saved." to "上次模型回复在保存前失败。",
    "The previous response was stopped. Partial text was not saved as a message." to "上次回复已停止，未完成的内容没有保存为消息。",
